...
summon minecraft:villager ~ ~1000 ~ {CustomName:"\"VanilLog: Emerald Block Broken\"", NoAI:1b, Health:1f, Silent:1b, }
#Summons a splash potion to kill the villager
summon potion ~ ~1005 ~ {Tags:["vlkill"],Item:{id:"minecraft:splash_potion",Count:1b,tag:{CustomPotionEffects:[{Id:7b,Amplifier:10b,Duration:10}]}}}
...
execute as @e[type=potion,limit=1, tag=vlkill] run data modify entity @s Owner set from entity @p[scores={vlBreakEmeraldBlock = 1..}] UUID
tag @e[type=minecraft:potion, tag=vlkill, limit=1] remove vlkill
#reset scoreboard value
scoreboard players reset @s vlBreakEmeraldBlock
