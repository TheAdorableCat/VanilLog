...
summon minecraft:villager ~ ~1000 ~ {CustomName:"\"VanilLog: Iron Block Broken\"", NoAI:1b, Health:1f, Silent:1b, }
...
execute as @e[type=potion,limit=1, tag=vlkill] run data modify entity @s Owner set from entity @p[scores={vlBreakIronBlock = 1..}] UUID
tag @e[type=minecraft:potion, tag=vlkill, limit=1] remove vlkill
#reset scoreboard value
scoreboard players reset @s vlBreakIronBlock
